...
void ViewManager::sessionFinished(Session *session)
{
    ...
    // Collect ALL views for this session (handles split-view case where
    // multiple TerminalDisplays share the same session).
    const QList<TerminalDisplay *> views = _sessionMap.keys(session);
    for (auto *v : views) {
        _sessionMap.remove(v);
    }
    ...
    // Use the first view for splitter/focus operations
    ...
    if (!_sessionMap.empty()) {
        for (auto *v : views) {
            updateTerminalDisplayHistory(v, true);
        }
        focusAnotherTerminal(splitter->getToplevelSplitter());
    }

    // Disconnect ALL views from the session's emulation BEFORE queuing
    // deleteLater.  SessionManager::sessionTerminated also connects to
    // Session::finished and queues session->deleteLater().  Because that
    // connection was made first, the session (and its Emulation/Screen
    // objects) will be destroyed before the views.  If any view still
    // references the ScreenWindow / Screen at that point, any deferred
    // event (paint, blink-timer) will dereference a dangling pointer and
    // SEGV.  Hiding each view and clearing its screen-window prevents this.
    for (auto *v : views) {
        v->hide();
        v->setScreenWindow(nullptr);
        v->deleteLater();
    }
...
void ViewManager::setCurrentView(TerminalDisplay *view)
{
    ...
    view->setFocus();
    if (_sessionMap.contains(view) && _sessionMap[view]) {
