...
- **Light suite** (run frequently): `bash tools/gate.sh` — **~24s with a warm build** (measured 23.5s, 45 tests). It compiles first, so after a pull or a header change the build dominates and this figure does not apply; a cold build is minutes. Time it yourself before setting any timeout — an unqualified runtime number in a doc is how a foreground run walks into a harness ceiling.
  - Prefer it over raw `ctest`: a filter that matches nothing, a typo'd filter, and an unconfigured build dir **all exit 0**, so the bare command reports success having verified nothing. The gate also refuses when a `*Test.cpp` exists that no CMakeLists registers — an unregistered test cannot fail.
  - **The filter lives in exactly one place**: `DEFAULT_LIGHT` in `tools/gate.sh`. Print it with `sed -n "s/^DEFAULT_LIGHT='\(.*\)'$/\1/p" tools/gate.sh` rather than copying it here. This file and `.claude/commands/test-light.md` both used to carry their own copies; the command's copy drifted 14 terms behind and was silently running 31 tests instead of 45 for months. Don't hand-write the count either — it said 40 while the tree had 41.
  - **Adding a test file is not enough — the filter must match its name.** `HookCleanupRaceTest`, `RemoteSshArgsTest`, `SessionLinkFilterTest` and `YoloPollingTest` existed, passed, and were skipped by this gate because no prefix matched them. The gate now refuses a `*Test.cpp` that CMake doesn't register, but it cannot tell you about a registered test the *filter* misses. Check `ctest -N` vs `ctest -N -R "<filter>"` when adding one.
